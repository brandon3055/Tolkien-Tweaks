...
public class PacketPlaceItem implements IMessage {
    ...
    public static class Handler extends MessageHandlerWrapper<PacketPlaceItem, IMessage> {
        ...
        public IMessage handleMessage(PacketPlaceItem message, MessageContext ctx) {
            if (!TTFeatures.placedItem.isBlockEnabled()) {
                return null;
            }
            ...
            if (traceResult != null && traceResult.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                if (tileHit instanceof TilePlacedItem && InventoryUtils.insertItem(((TilePlacedItem) tileHit).inventory, stack, true) == 0) {
                    ...
                    BlockEvent.PlaceEvent event = ForgeEventFactory.onPlayerBlockPlace(player, new BlockSnapshot(world, pos, TTFeatures.placedItem.getDefaultState()), traceResult.sideHit, player.getActiveHand());

                    if (event.isCanceled()) {
                        return null;
                    }

                    world.setBlockState(traceResult.getBlockPos().offset(traceResult.sideHit), TTFeatures.placedItem.getDefaultState().withProperty(PlacedItem.FACING, traceResult.sideHit.getOpposite()));
