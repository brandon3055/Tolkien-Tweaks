...
public class CamoSpawner extends ChameleonBlock<TileCamoSpawner> {
    ...
    @Override
    public TileCamoSpawner createChameleonTile(World worldIn, int meta) {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (tile instanceof TileCamoSpawner) {
            if (player.capabilities.isCreativeMode && ((TileCamoSpawner) tile).attemptSetFromStack(player.getHeldItem(hand))) {
                return true;
            }
            else {
